...
class State:
    ...
    def __post_init__(self) -> None:
        ...
        # Keep `turn` and `turn_number` synchronized while we migrate callers to `turn_number`.
        if self.turn_number is None and self.turn is not None:
            try:
                self.turn_number = int(self.turn)
            except (TypeError, ValueError):
                self.turn_number = None
        elif self.turn is None and self.turn_number is not None:
            try:
                self.turn = int(self.turn_number)
            except (TypeError, ValueError):
                self.turn = None
        elif self.turn is not None and self.turn_number is not None:
            try:
                self.turn = int(self.turn_number)
            except (TypeError, ValueError):
                self.turn = None
    ...
    @property
    def revealed_opponent_team(self) -> tuple[PokemonSnapshot, ...]:
...
def parse_battle_to_state(battle: Any) -> State:
    ...
    forced_switch = _extract_forced_switch(battle)
...
def _extract_forced_switch(battle: Any) -> bool:

    force_switch = getattr(battle, "force_switch", False)
    if isinstance(force_switch, (list, tuple, set)):
        return any(bool(item) for item in force_switch)
